...
1. Create a new, empty migration file: `cd ghost/core && bun run migrate:create <kebab-case-slug>`. IMPORTANT: do not create the migration file manually; always use this script to create the initial empty migration file. The slug must be kebab-case (e.g. `add-column-to-posts`).
...
5. Test the migration manually: `cd ghost/core && bun run knex-migrator migrate --v {version directory} --force`
...
8. Run the schema integrity test, and update the hash: `cd ghost/core && bun run test:single test/unit/server/data/schema/integrity.test.js`
9. Run unit tests in Ghost core, and iterate until they pass: `cd ghost/core && bun run test:unit`
