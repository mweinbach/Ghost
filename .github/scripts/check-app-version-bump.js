...
function main() {
    ...
    for (const app of changedApps) {
        ...
        if (compareSemver(prVersion, mainVersion) <= 0) {
            failedApps.push(
                `${app.key} (${app.packageName}) was changed but version was not bumped above main (${prVersion} <= ${mainVersion}). Please run "bun run ship" in ${app.path} to bump the package version.`
            );
            continue;
        }
