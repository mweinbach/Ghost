...
if [[ "$GHOST_E2E_MODE" == "dev" ]]; then
  ...
  echo "  Tip: For local development, run 'bun run dev' first — dev mode is faster and doesn't require a pre-built Docker image."
fi
...
if [[ "$GHOST_E2E_MODE" == "dev" ]]; then
  bun run --filter @tryghost/parse-email-address build >/dev/null
fi
