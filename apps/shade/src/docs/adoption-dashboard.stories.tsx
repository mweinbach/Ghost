...
const meta = {
    ...
    parameters: {
        ...
        docs: {
            description: {
                component:
                    'A snapshot of which design systems each Ghost admin surface uses today. The goal isn\'t to maximize Shade imports — many files (utilities, hooks, contexts) legitimately don\'t need a design system. The useful signal is **legacy retirement progress**: how many files still depend on `@tryghost/admin-x-design-system`, and which specific exports are blocking their migration. Data is generated by `bun run --filter @tryghost/shade adoption:extract` and committed as `adoption-data.json`.'
            }
        }
    }
