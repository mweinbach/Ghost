...
export class UrlServiceFacade {
    ...
    // Base-filter columns a resource of this type must carry for the lazy
    // backend to decide its URL. [] with no lazy backend: eager looks URLs up
    // by id and never reads these fields.
    getRequiredFields(routerType: string): string[] {
        if (this.lazyUrlService) {
