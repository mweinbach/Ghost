...
Run all tests in the browser by running `bun run dev` in the Ghost monorepo and visiting http://localhost:4200/tests. The code is hotloaded on change and you can filter which tests to run.
...
```bash
bun run build --environment=test -w -o="dist-test"
```

Then run tests with:

```bash
TZ=UTC bun run test 1 --reporter dot --path="dist-test"
```
...
```bash
TZ=UTC bun run test 1 --reporter dot --path="dist-test" -mp=tests/unit/helpers/gh-count-characters-test.js
```
